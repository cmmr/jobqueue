...
| State          | Triggers                                                |
|----------------|---------------------------------------------------------|
| `'starting'`   | After starting a new `callr::r_session` process.        |
...
# A single worker to best illustrate processing order.
q <- Queue$new(
  'workers' = 1, 
...
In the first example, we attached a hook to `'submitted'` because that's when `<Job>$queue` becomes available in callbacks. In the latter example, we attached a hook to `'created'` instead because we didn't need `<Job>$queue` for that solution. Check the trigger order listed in the "Callback Triggers" section above, and attach callbacks as early on as possible to expedite Job handling.
