...
You can save compute resources by stopping the execution of Jobs that are no longer needed. If that Job is currently running on a Worker, then the associated `callr::r_session` background process is terminated, and a replacement process is spun up to take its place in the Worker pool.
...
```r
job <- Job$new({ Sys.sleep(1); 'Zzzzz' }, timeout = 0.2)
q$submit(job)
job$result
