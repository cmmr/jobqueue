...
However, while `<Job>$output` is a fixed value, `<Job>$result` is highly configurable. Most notably by the `reformat` and `signal` arguments.
...
```r
all_output <- function (job) {
  list(result = job$output, stdout = job$stdout, stderr = job$stderr)
}

job <- q$run({ cat('hi'); message('hey'); 42 }, reformat = all_output)
dput(job$result)
#> list(result = 42, stdout = "hi", stderr = "hey")
```
...
```r
job <- q$run({ 42 }, signal = 'error')$stop()
