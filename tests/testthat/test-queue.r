...
test_that('basic', {
  ...
  expect_error( Queue$new(init = { stop() }, wait = 60, workers = 1L))
  expect_error( Queue$new(init = { q()    }, wait = 60, workers = 1L))
})
...
test_that('interrupt', {
  ...
  q <- expect_silent(Queue$new(workers = 1L, timeout = 10, wait = 60))
